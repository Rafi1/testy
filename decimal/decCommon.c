  ...
  if (!NUMISSPECIAL(num)) {
    ...
    if (drop>0) {                            // rounding needed
      ...
      if (reround!=0) {                      // discarding non-zero
        ...
        // if adjusted exponent [exp+digits-1] is < EMIN then we are
        // subnormal and hence raise Underflow
        if (num->exponent<DECEMIN && (num->exponent+(ulsd-umsd+1)-1)<DECEMIN)
          set->status|=DEC_Underflow;

        // next decide whether we need to increment the coefficient
        ...
        if (bump!=0) {                       // need increment
          // increment the coefficient; we might end up with 1000...
          ...
          if (ub<umsd) {                     // had all-nines
            *umsd=1;                         // coefficient to 1000...
            // usually we can use the 1000... coefficient as-is
            if ((ulsd-umsd+1)==DECPMAX) {
              num->exponent++;
              }
             else {
              // if coefficient is shorter than Pmax then we have a
              // subnormal, so extend it; this is safe as drop>0
              // (or, if we supplied the coefficient, it could not be 9)
              // This may make the result normal.
              ulsd++;
              *ulsd=0;
              // [exponent unchanged]
              #if DECCHECK
    ...
    if (num->exponent>DECEMAX-(DECPMAX-1)) { // is edge case
      ...
      if (*ulsd==0 && ulsd==umsd) {     // have zero
        ...
        if (shift>0) {                  // fold-down needed
          // fold down needed; we have to copy to buffer in order to
          // pad with zeros safely; fortunately this is not worst case
          // path because we cannot have had a round
...
decFloat * decFloatFromString(decFloat *result, const char *string,
  ...
  uByte  buffer[ROUNDUP(DECSTRING+6, 8)]; // room for most coefficents
                                        // and some common rounding, & pad
  #if DECTRACE
  // printf("FromString %s ...\n", string);
  #endif
  ...
  for(;;) {                             // once-only 'loop'
    ...
    // The test for four-at-a-time here could, in theory, cause a
    // storage violation; we can avoid this (and may have to, later)
    // by only doing the test when c is on a word alignment.  This
    // is measureably slower.
    for (c=string;; c++) {              // -> input character
      // try for four valid digits at a time
      if ( ((UINTAT(c)&0xf0f0f0f0)==CHARMASK)                // OK masks
       && (((UINTAT(c)+0x06060606)&0xf0f0f0f0)==CHARMASK)) { // all <=9
        c+=3;                           // [will be 4 in total]
        continue;
        }
    ...
    if (digits>0) {                     // had digits and/or dot
      ...
      if (*c!='\0') {                   // something follows the coefficient
        ...
        firstexp=c;                     // remember where we are [maybe '\0']
      ...
      // then we can simply convert to bcd8 and copy -- decFinalize
      ...
      if (digits<=(Int)(sizeof(buffer)-3)) { // [-3 allows by-4s copy]
        ...
        if (dotchar!=NULL) {                 // a dot to worry about
          if (*(c+1)=='.') {                 // common canonical case
            *ub++=(uByte)(*c-'0');           // copy leading digit
            ...
            // as usual, go by fours when we can; note we have asserted
            // that a '.' does not have the same mask as a digit
            // (this also notices if the terminator is in the four)
            if ((UINTAT(c)&0xf0f0f0f0)==CHARMASK) { // four digits
        ...
        // Now no dot; do this by fours (may move 3 more than needed)
        for (; c<=clast; c+=4, ub+=4) UINTAT(ub)=UINTAT(c)&0x0f0f0f0f;
...
char * decFloatToEngString(const decFloat *df, char *string){
  ...
  // the remaining ones are full 3-char length.  We use fixed-length
  // copies because variable-length memcpy causes a subroutine call
  ...
  if (pre>0) {                     // ddd.ddd (plain), perhaps with E
    ...
    // for Quads and Singles we might have a character or two left...
...
///* non-0 exponent */
  ...
  // rightmost character that we write must not be beyond where the
