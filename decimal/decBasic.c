  ...
  for (;; lsua--) {           // outer loop -- each input position
    ...
    *lsuq=0;                  // default unit result is 0
    for (;;) {                // inner loop -- calculate quotient unit
      ...
      // minimise the iterations of this inner loop; the subtractions
      // needed dominate division performance.
      if (accunits==divunits) {
...
//#undef  DECUSE64
//#define DECUSE64 0       // local for testing
//#undef  DECTRACE
//#define DECTRACE 1       // local for testing
...
static void decFiniteMultiply(bcdnum *num, uByte *bcdacc,
  ...
  for (pa=acc+MULACCLEN-1; pa>=acc; pa--)
    printf(" %08x:%08x", *(pa+MULACCLEN), *pa);
  printf("\n");
  #endif
...
// next one is used when we know the declet must be non-zero, or is
// the final zero declet
...
decFloat * decFloatQuantize(decFloat *result,
  ...
  // the coefficient is needed; lay it out into buf, offset so we can
  // add zeros before or after as needed -- an extra heading is
  // added so we can safely pad Quad DECPMAX-1 zeros to the left by
  // fours
